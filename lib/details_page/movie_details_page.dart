...
class _MovieDetailsPageState extends State<MovieDetailsPage> {
  ...
  Future<void> movieDetails() async {
    // Movie Details
    var movieDetailsResponse = await http.get(
        Uri.parse(ApiLink.itemDetailsUrl(widget.itemId.toString(), 'movie')));
    ...
    if (movieDetailsResponse.statusCode == 200) {
      ...
      print(
          "Error: ${movieDetailsResponse.statusCode}:${movieDetailsResponse.reasonPhrase}");
    }

    // Similar Movies
    var similarMoviesResponse = await http.get(
        Uri.parse(ApiLink.itemSimilarUrl(widget.itemId.toString(), 'movie')));
    ...
    if (similarMoviesResponse.statusCode == 200) {
      ...
      var similarMoviesJsonResults = jsonDecode(similarMoviesResponse.body)[
          'results']; // replace with tempData['results'];
      ...
      for (var i = 0; i < similarMoviesJsonResults.length; i++) {
        similarItems.add({
          ...
          'vote_average': similarMoviesJsonResults[i]['vote_average'] ?? 0,
      ...
      print(
          "Error: ${similarMoviesResponse.statusCode}:${similarMoviesResponse.reasonPhrase}");
    }

    // Recommended Movies
    var recommendedMoviesResponse = await http.get(Uri.parse(
        ApiLink.itemRecommendationsUrl(widget.itemId.toString(), 'movie')));
    ...
    if (recommendedMoviesResponse.statusCode == 200) {
      ...
      var recommendedMoviesJsonResults =
          jsonDecode(recommendedMoviesResponse.body)[
              'results']; // replace with tempData['results'];
      ...
      print(
          "Error: ${recommendedMoviesResponse.statusCode}:${recommendedMoviesResponse.reasonPhrase}");
    }

    // User Reviews
    var userReviewResponse = await http.get(
        Uri.parse(ApiLink.itemReviewsUrl(widget.itemId.toString(), 'movie')));
    ...
    if (userReviewResponse.statusCode == 200) {
      ...
      var userReviewJsonResults = jsonDecode(userReviewResponse.body)[
          'results']; // replace with tempData['results'];
      for (var i = 0; i < userReviewJsonResults.length; i++) {
        userReviews.add({
          ...
          'rating': userReviewJsonResults[i]['author_details']['rating'] == null
              ? "Not Rated"
              : userReviewJsonResults[i]['author_details']['rating'].toString(),
          'profileImage': userReviewJsonResults[i]['author_details']
                      ['avatar_path'] ==
                  null
              ? "https://www.pngitem.com/pimgs/m/146-1468479_my-profile-icon-blank-profile-picture-client.png"
              : ApiConstants.baseImageUrl +
                  userReviewJsonResults[i]['author_details']['avatar_path'],
      ...
      print(
          "Error: ${userReviewResponse.statusCode}:${userReviewResponse.reasonPhrase}");
    }

    // Movie Trailers
    var movieTrailersResponse = await http.get(
        Uri.parse(ApiLink.itemTrailerUrl(widget.itemId.toString(), 'movie')));

    if (movieTrailersResponse.statusCode == 200) {
      //var tempData = jsonDecode(movieTrailersResponse.body);
      var movieTrailersJsonResults = jsonDecode(movieTrailersResponse.body)[
          'results']; // replace with tempData['results'];
      ...
      print(
          "Error: ${movieTrailersResponse.statusCode}:${movieTrailersResponse.reasonPhrase}");
    }
  }
  ...
  void initState() {
    super.initState();
    checkConnectivity(
        context); // Call checkConnectivity when the widget is initialized
  }
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      bottomNavigationBar:
          const CustomNavigationBar(currentState: "MovieDetailsPage"),
                  ...
                  slivers: [
                    ...
                    SliverList(
                        ...
                        children: [
                          ...
                          ResponsiveWidget(
                            mobile: Column(
                                ...
                                children: [
                                  ListActionButton(
                                      "Add to Watch List", Icons.add, () {
                                    addToWatchList(widget.itemId, context);
                                  }),
                                  ListActionButton(
                                      "Add to Watched List", Icons.add, () {
                                    addToWatchedList(widget.itemId, context);
                                  }),
                                ]),
                            desktop: Row(
                                mainAxisAlignment: MainAxisAlignment.center,
                      ...
                      Padding(
                        padding: const EdgeInsets.only(left: 20, top: 10),
                        child: UserReview(
                            userReviews), //Text(userReviews.isNotEmpty.toString()),
                      ),
                      ...
                      Padding(
                        ...
                        child: Text(
                            'Release Date : ' +
                                itemDetails[0]['release_date'].toString(),
                      ...
                      Padding(
                        ...
                        child: Text(
                            'Budget : \$' + itemDetails[0]['budget'].toString(),
                            style: Theme.of(context).textTheme.headlineLarge),
                      ),
                      ...
                      Padding(
                        ...
                        child: Text(
                            'Revenue : \$' +
                                itemDetails[0]['revenue'].toString(),
                            style: Theme.of(context).textTheme.headlineLarge),
                      ),

                      similarItems.isNotEmpty
                          ? SliderList(similarItems, "More Movies Like This",
                              itemDetails[0]['media_type'], similarItems.length)
